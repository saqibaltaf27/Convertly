...
/**
 * ExcelToWord component
 * - Drag & drop or click to pick multiple .xls/.xlsx files
...
export default function ExcelToWord() {
  ...
  const addFiles = (fileList) => {
    ...
    if (!incoming.length) return alert("Please select .xls or .xlsx files");
  ...
  const convertSingle = async (item) => {
    ...
    try {
      ...
      // update progress and set download link
      updateItem(item.id, {
        progress: 100,
        status: "done",
  ...
  const convertAll = async () => {
    if (!items.length) return alert("Please add Excel files to convert.");
    setIsConverting(true);

    // Kick off conversions in parallel
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white pt-24 px-6">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl font-extrabold text-blue-700 mb-2">Excel → Word</h1>
        <p className="text-gray-600 mb-8">
          Drag & drop multiple Excel files (.xls, .xlsx) or click to select. Each file will be converted
        ...
        <div
          ...
          aria-label="Drop Excel files here"
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-left">
                <div className="font-medium text-gray-800">
                  {dragging ? "Drop files to upload" : "Drag & drop Excel files here"}
                </div>
                <div className="text-sm text-gray-500">
                  Click to browse — supports .xls and .xlsx — multiple files allowed
                </div>
              </div>
            </div>
